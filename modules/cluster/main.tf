...
resource "kubernetes_secret" "jx-post-process" {
  ...
  data = {
    commands : <<EOF
kubectl annotate -n jx --overwrite sa tekton-bot eks.amazonaws.com/role-arn=arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.cluster_name}-jx-tekton-bot
kubectl annotate -n jx --overwrite sa jxboot-helmfile-resources-controllerbuild eks.amazonaws.com/role-arn=arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.cluster_name}-jx-jxboot-helmfile-resources-controllerbuild
EOF
  }
